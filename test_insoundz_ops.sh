...
ARGS="--jobs ${JOBS} -c opt --copt=-mavx --copt=-mavx2 --copt=-mfma --copt=-mfpmath=both --config=mkl --cxxopt=-DEIGEN_MKL_DEFAULT --cxxopt=-DEIGEN_USE_MKL_ALL $DBG"
#bazel test //tensorflow/core/user_ops/insoundz_ops/tf_insoundz/rankoneupdate:rankoneupdate_ops_py_test
bazel test //tensorflow/python/kernel_tests:rnn_test
